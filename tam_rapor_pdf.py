...
# Kod bloklari ve satir ici kod icin Consolas. reportlab'in gomulu
# "Courier" fontunda TURKCE HARF YOK: "varsayimsal" kelimesindeki
# noktasiz i, PDF'te siyah kutu olarak cikiyordu (yillar once Arial'i
# bu yuzden kaydetmistik, ayni tuzak monospace tarafinda tekrarladi).
...
# Kod blogu satirlarini isaretlemek icin; rapor metninde gecmeyecek
# diziler. Null bayt kullanilmiyor -- dosyayi ikili yapiyor.
KOD_ISARET = "@@KOD@@"
KOD_SINIR = "@@SINIR@@"
...
def paragraflari_birlestir(satirlar: list[str]) -> list[str]:
    ...
    cikti: list[str] = []
    tampon: list[str] = []
    # Fenced kod bloklari (```) OLDUGU GIBI korunmali. Onceki surum
    # bunlari duz metin sayip tek paragrafa duzlestiriyordu: akis
    # semalari "` avukat bir soru yazar | v +" gibi okunmaz bir satira
    # doniyordu.
    blokta = False
    ...
    for ham_satir in satirlar:
        ...
        if s.strip().startswith("```"):
            bosalt()
            blokta = not blokta
            cikti.append(KOD_SINIR)
            continue
        if blokta:
            cikti.append(KOD_ISARET + s)
            continue
        if not s:
            bosalt()
            cikti.append("")
...
def main() -> None:
    ...
    def kodu_bosalt():
        """Kod blogunu tek parca, tek arka planli kutu olarak cizer.

        Satir satir Paragraph uretmek sayfa sonunda blogu ortadan
        boluyordu; akis semasi iki sayfaya yayilinca okunmuyor. Tek
        hucreli tablo blogu birlikte tutuyor.
        """
        nonlocal kod_tampon
        if not kod_tampon:
            return
        # Bosluklar korunmali: seffaf bosluk yerine sabit bosluk.
        govde = "<br/>".join(
            x.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
             .replace(" ", "&nbsp;")
            for x in kod_tampon)
        stil = ParagraphStyle("kod", fontName="Mono", fontSize=7.6,
                              leading=9.8, textColor=colors.HexColor("#20252b"))
        t = Table([[Paragraph(govde, stil)]], colWidths=[16.8 * cm])
        t.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#f4f5f6")),
            ("BOX", (0, 0), (-1, -1), 0.4, colors.HexColor("#dcdfe3")),
    ...
    for satir in satirlar:
        ...
        if s == KOD_SINIR:
            tabloyu_bosalt(); listeyi_bosalt(); kodu_bosalt()
            continue
        if s.startswith(KOD_ISARET):
            kod_tampon.append(s[len(KOD_ISARET):])
            continue
